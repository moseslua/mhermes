...
If you can't use the wrapper (e.g. on Windows or inside an IDE that shells pytest directly), at minimum activate the venv, clear `pyproject.toml` addopts, and pin `-n 4`:

```bash
source venv/bin/activate
python -m pytest -o "addopts=" tests/ -q -n 4 --ignore=tests/integration --ignore=tests/e2e -m "not integration"
```
...
```
Run the default suite before pushing changes, and add explicit integration/e2e coverage separately whenever your change touches those paths.
